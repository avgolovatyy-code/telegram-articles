...
## Шаг 5. Подготовить домен — 5 минут

Нужен для https на ссылках трекинга кликов. Заведите поддомен, например
`content.wegotrip.com`, и добавьте A-запись на IP сервера. IP появится на шаге 7,
поэтому пока просто решите, какой поддомен использовать.

Если домена нет и не хочется — выбирайте **вариант B** (App Platform), там DigitalOcean
сам выдаёт адрес `*.ondigitalocean.app` с сертификатом.
...
Дальше два пути, выберите один.

**Вариант «делаю я».** Добавьте секреты в
[cursor.com/dashboard](https://cursor.com/dashboard) → Cloud Agents → **Secrets**
и напишите мне следующим сообщением — я подниму сервер и всё настрою.

| Переменная | Значение |
| --- | --- |
| `DIGITALOCEAN_ACCESS_TOKEN` | токен из шага 6 |
| `OPENAI_API_KEY` | ключ из шага 4 |
| `TELEGRAM_BOT_TOKEN` | токен из шага 1 |
| `TELEGRAM_TEST_CHANNEL` | `@wegotrip_content_test` из шага 2 |
| `DEPLOY_DOMAIN` | поддомен из шага 5 |
| `ACME_EMAIL` | ваша почта для Let's Encrypt |
| `DEPLOY_SSH_PRIVATE_KEY` | приватный ключ, чья публичная часть загружена в DO |

> Секреты шифруются и подставляются только в **новые** запуски агента, поэтому после
> добавления обязательно напишите мне. **Не присылайте токены текстом в чат**: всё, что
> попало в переписку, считается скомпрометированным и подлежит отзыву.

**Вариант «делаю сам».** Токены мне не нужны, пройдите раздел
[«Вариант A»](#вариант-a-droplet--docker-compose) ниже — там готовые скрипты, вся
установка сводится к трём командам.

## Шаг 8. Приёмка — 20 минут

После запуска, до включения автопубликации:

1. Откройте `https://<домен>/admin` — логин `admin`, пароль из `ADMIN_PASSWORD`.
2. Проверьте на дашборде: каталог EN и RU не пустой, есть кандидаты тем.
3. Откройте любую статью → **Publish to test** → посмотрите сообщение в тест-канале:
   заголовки, фото, карточки товаров, кнопки.
4. Нажмите кнопку товара и убедитесь, что в адресе есть `coupon=435`.
5. Прочитайте текст. Если тон или факты не устраивают — скажите мне, поправим промпты.
6. Понравилось — **Approve** и **Publish now** в продакшен.
7. Понаблюдайте день-два в режиме ревью, потом включайте автопубликацию по одному
   рынку: `AUTO_PUBLISH_RU=true`, через несколько дней `AUTO_PUBLISH_EN=true`.
...
| `DEPLOY_DOMAIN` | вариант A | шаг 5 | https для ссылок трекинга |
