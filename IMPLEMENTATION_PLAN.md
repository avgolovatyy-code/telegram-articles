...
| `discover_topics` | 03:00 | Кандидаты тем по всем уровням каталога, скоринг, дедупликация |
| `generate_daily_articles` | 04:00, 10:00, 16:00 | Генерация в рамках дневного плана бюджета |
| `schedule_publications` | 05:30, 11:30, 17:30 | Равномерное распределение публикаций |
...
`DAILY_AI_BUDGET_USD=3.00` — жёсткий предел на LLM, web search и генерацию изображений.

* `cost_ledger` — фактические расходы по данным `usage` из API, не прогноз.
...
* `plan_daily_generation()` — сначала минимумы (10 EN + 10 RU) по одной статье
  поочерёдно, чтобы ни один рынок не голодал, потом рост до максимумов (20 + 20).
* `can_start_article()` — отказ, если прогноз превышает остаток минус запас.
* Публикация уже готовых статей бюджетом не блокируется.

Оценка при 12k символов контекста и 10k символов вывода ≈ $0.06–0.09 на статью, что
укладывается в $3 на 30–40 статей.
