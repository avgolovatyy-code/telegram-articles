...
| `wgt slack-check [--post]` | Проверка Slack-токена; `--post` шлёт тестовое сообщение |
| `wgt telegram-chats` | Показать id каналов, включая приватные без `@username` |
| `wgt secrets set/list/rm` | Шифрованное хранилище ключей |
...
* Ключи шифруются на диске (Fernet, AES-128-CBC + HMAC): `wgt secrets set OPENAI_API_KEY`.
  Мастер-ключ — отдельный файл с правами `600`, расшифровка только в память при старте.
* Логи маскируют и известные значения секретов, и всё, что похоже на ключ по форме.
* `.env` содержит только несекретные настройки и лежит в `.gitignore`.
